...
class OpenEduCatLms(CustomerPortal):
    ...
    def get_material(self, material_id, **kw):
        if material_id:
            material = request.env['op.material'].sudo().browse([
                int(material_id)])
            return {'embed_code': material.embed_code, 'name': material.name}
    ...
    def courses(self, search='', category=False, page=0, ppg=False, **post):
        ...
        courses = course_ref.sudo().search(course_domain, order='sequence asc')
    ...
    def get_certificate_overview(
        ...
        searchbar_inputs = {
            'content': {'input': 'content',
                        'label': _('Search in Index <span class="nolabel">'
                                   ' (in Content)</span>')},
