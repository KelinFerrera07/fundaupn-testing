...
class OpCircular(models.Model):
    ...
    def get_circular_emails(self):
        email_ids = ''
        for student in self.group_id.student_ids:
            if student.partner_id.email:
                email_ids = email_ids + ',' + str(student.sudo().partner_id.email)
            else:
                email_ids = str(student.sudo().partner_id.email)

        for parent in self.group_id.parent_ids:
            if parent.partner_id.email:
                email_ids = email_ids + ',' + str(parent.sudo().partner_id.email)
            else:
                email_ids = str(parent.sudo().partner_id.email)

        for faculty in self.group_id.faculty_ids:
            if faculty.partner_id.email:
                email_ids = email_ids + ',' + str(faculty.sudo().partner_id.email)
            else:
                email_ids = str(faculty.sudo().partner_id.email)
        return email_ids
