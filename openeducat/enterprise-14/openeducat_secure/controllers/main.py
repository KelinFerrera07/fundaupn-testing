...
class SecureQRCode(http.Controller):
    ...
    def binary_data(self, **post):
        if post:
            qr_code = request.env['ir.config_parameter'].sudo().get_param('qr_code')
            ...
            if data:
                for m in data:
                    ...
                    if res == qr_binary:
                        ...
                        v_model = 'ir.ui.view'
                        return request.env[v_model].sudo()._render_template(view.id, {
                            'single': singular_fields,
                            'multiple': multiple_fields,
                            'type': qr_code, })

        return request.env['ir.ui.view'].sudo(). \
            _render_template('openeducat_secure.error_page', {'type': qr_code})
    ...
    def general_data(self, hash_key=None, **kwargs):
        ...
        return request.render('openeducat_secure.error_page', {'type': qr_code})
