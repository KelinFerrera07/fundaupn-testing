...
class Grievance(models.Model):
    ...
    def _composer_format(self, res_model, res_id, template):
        ...
        team_record = self.env['grievance.team'].browse(self.grievance_team_id.id)
        partner = ''
        for g in team_record:
            partner = str(g.team_leader.partner_id.id)
            partner_emails = str(g.team_leader.partner_id.email)
            for mem in g.member_ids:
                partner = partner + ',' + str(mem.partner_id.id)
                partner_emails = partner_emails + ',' + str(mem.partner_id.email)
        ctx = dict(
